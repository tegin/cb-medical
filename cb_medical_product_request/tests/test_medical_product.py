...
class TestMedicalProduct(TransactionCase):
    ...
    def test_name_search(self):
        ...
        self.assertEqual(
            search_by_lab_name[0][0], self.test_product_template.id
        )
    ...
    def test_action_view_product_tmpl_commercial_ids(self):
        self.assertEqual(
            self.test_product_template.product_tmpl_commercial_count, 1
        )
        action = (
            self.test_product_template.action_view_product_tmpl_commercial_ids()
        )
        self.assertEqual(action["res_id"], self.test_template_lab.id)

    def test_action_view_product_commercial_ids(self):
        self.assertEqual(
            self.test_product_30_tablets.product_commercial_count, 1
        )
        action = (
            self.test_product_30_tablets.action_view_product_commercial_ids()
        )
        self.assertEqual(
            action["res_id"], self.test_template_30_tablets_lab.id
        )
