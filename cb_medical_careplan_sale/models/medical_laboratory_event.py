...
class LaboratoryEvent(models.Model):
    ...
    is_sellable_insurance = fields.Boolean(compute="_compute_is_sellable", store=True)
    is_sellable_private = fields.Boolean(compute="_compute_is_sellable", store=True)
    ...
    def get_sale_order_query(self):
        query = []
        for event in self.filtered(lambda r: r.fhir_state not in ["aborted"]):
